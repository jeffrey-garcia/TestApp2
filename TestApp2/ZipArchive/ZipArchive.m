...
//	zipInfo.dosDate = (unsigned long) current;
    ...
    NSDictionary* attr = [[NSFileManager defaultManager] attributesOfItemAtPath:file error:&fileError];
	if( attr )
	{
...
-(BOOL) UnzipOpenFile:(NSString*) zipFile
{
	...
	if( _unzFile )
	{
		...
		{
			//DebugLog(@"%@",[NSString stringWithFormat:@"%d entries in the zip file",globalInfo.number_entry] );
		}
	}
		...
		if( fp )
		{
			...
			if( attr )
			{
				...
				if( ![[NSFileManager defaultManager] setAttributes:attr ofItemAtPath:fullPath error:nil] )
				{
					...
					//DebugLog(@"Failed to set attributes");
				}
				
			}
		
			//DebugLog(@"unzip file Path:%@",fullPath);
